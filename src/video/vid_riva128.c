		...
		uint32_t graphobj3, void *p)
{
	...
	if(objclass != 0x1c && objclass != 0x0c && objclass != 0x02 && objclass != 0x05) pclog("[RIVA 128] PGRAPH execute grobj0 %08x objclass %02x method %04x param %08x\n", graphobj0
		...
		else if(method >= 0x1400 && method < 0x1480)
		{
			...
			&& riva128->pgraph.gdi_cur_y_e <= riva128->pgraph.gdi_clip_bottom_e)
			{
				for(int bit = 7; bit >= 0; bit--)
				{
					...
					if(riva128->pgraph.gdi_cur_x_e >= riva128->pgraph.gdi_clip_right_e || riva128->pgraph.gdi_cur_x_e >= (riva128->pgraph.gdi_vtx_x_e + riva128->pgraph.gdi_vtx_w_e))
					{
						riva128->pgraph.gdi_cur_x_e = riva128->pgraph.gdi_vtx_x_e;
						riva128->pgraph.gdi_cur_y_e++;
						if(riva128->pgraph.gdi_cur_y_e >= riva128->pgraph.gdi_clip_bottom_e || riva128->pgraph.gdi_cur_y_e >= (riva128->pgraph.gdi_vtx_y_e + riva128->pgraph.gdi_vtx_h_e))
							goto method_end;
					}
				}
    ...
    case 0x0d:
        switch(method) {
			...
			if (target)
			{
				...
				{
					uint32_t bytes = riva128->pgraph.m2mf_pitch_out > riva128->pgraph.m2mf_scan_len ? riva128->pgraph.m2mf_pitch_out : riva128->pgraph.m2mf_scan_len;
					for(uint32_t pixel = 0; pixel < bytes; pixel += inc_in)
					{
						uint8_t buf = 0;
					...
					riva128->pgraph.m2mf_in_dma_cur += bytes;
					riva128->pgraph.m2mf_out_dma_cur += bytes;
				}
			}
			else
			{
	...
	case 0x15:
		...
		{
			pclog("[RIVA 128] SIFM x %08x y %08x w %08x h %08x dxdu %08x dydv %08x", riva128->pgraph.sifc_vtx_x, riva128->pgraph.sifc_vtx_y, riva128->pgraph.sifc_vtx_w_out, riva128->pgraph.sifc_vtx_h_out, riva128->pgraph.sifc_dx_du, riva128->pgraph.sifc_dy_dv);
			int final_x = 0;
			int final_y = 0;
